@php($items = $enrollments ?? collect())

<div class="space-y-3">
    @if($items->isEmpty())
        <div class="text-sm text-gray-500">Belum ada program yang diikuti.</div>
    @else
            ...
            @foreach($items as $en)
                <div class="flex items-start gap-3 rounded-md border border-gray-200 dark:border-gray-700 p-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-medium truncate">{{ $en->program?->title ?? '-' }}</div>
                        <div class="text-xs text-gray-500 flex flex-wrap items-center gap-2">
                            ...
                            @if($en->enrolled_at)
                                <span>Daftar: {{ optional($en->enrolled_at)->format('d M Y') }}</span>
                            @endif
                            @if($en->completed_at)
                                <span>· Selesai: {{ optional($en->completed_at)->format('d M Y') }}</span>
                            @endif
                        </div>
                        @if($en->notes)
