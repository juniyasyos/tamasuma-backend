<x-filament::section>
    <x-slot name="heading">{{ __('filament-breezy::default.profile.sanctum.title') }}</x-slot>
    <x-slot name="description">{{ __('filament-breezy::default.profile.sanctum.description') }}</x-slot>
    ...
    @if($plainTextToken)
        <div class="space-y-2 bg-warning-500 p-4 rounded">
            ...
            <div class="flex items-center justify-between">
                ...
                <x-filament::button icon="heroicon-s-clipboard-document-check" size="sm" type="button" wire:click="$set('plainTextToken',null)">{{ __('filament-breezy::default.profile.sanctum.copied.label') }}</x-filament::button>
            </div>
        </div>
    @endif
